/*
Sankarsan Seal
Batch Learning of Single Layer Perceptron
 */
#include <cstdio>
#include <cstdlib>
#include <iostream>
...
int totalerror[NO_OF_PARAMETER];
float averageerror[NO_OF_PARAMETER];
...
void signum(int i)
{
    ...
    int result=0;
	    	...
	    	for(j=0;j<NO_OF_PARAMETER;j++)
	    	totalerror[j]+=((pc[i].ylabel)-result)*(pc[i].x[j]);
...
void updateW()
{
    ...
    for(j=0;j<NO_OF_PARAMETER;j++)
    cout<<totalerror[j]<<endl;

    for(j=0;j<NO_OF_PARAMETER;j++)
    averageerror[j]=(float)totalerror[j]/(float)n;

    for(j=0;j<NO_OF_PARAMETER;j++)
    {

        w[j]+=eta*averageerror[j];

        //fprintf(stdout,"%d : x[%d]:%f  w[%d]:%f , signum: %d, result : %d, ylabel: %d\n"
        //				,i,j,pc[i].x[j],j,w[j], signum(i),result,pc[i].ylabel);
        cout<<w[j]<<" " << eta*averageerror[j]<<endl;
    }
